<?php
/**
 * USER AGENT BLOCKLIST PLUGIN
 *
 * This plugin and its blocklist is machine generated based on the user agent database at 
 * user-agents.org and robotstxt.org. See create-blocklist.php (not in trunk, so not normally
 * bundled with ShrimpTest) on how it is generated.
 *
 * @package ShrimpTest
 * @subpackage ShrimpTest_Plugin_Blocklist
 * @author mitcho (Michael Yoshitaka Erlewine) <mitcho@mitcho.com>, Automattic,
 *   autogenerated
 */

add_filter( 'shrimptest_blocked_visit', 'shrimptest_plugin_check_blocklist', 10, 2 );
/**
 * Returns true if the {@$user_agent} should be blocked.
 * @param bool
 * @param string
 * @return bool
...
function shrimptest_plugin_check_blocklist( $blocked, $user_agent ) {
	...
	$blockterms = apply_filters( 'shrimptest_blockterms', $blockterms );
	$blocklist = apply_filters( 'shrimptest_blocklist', $blocklist );

	if ( !is_array( $blockterms ) )
		$blockterms = array( 'this is a dummy string which should never match' );
	$blockterms = array_map( 'preg_quote', $blockterms );
	$blockterms_regexp = '%('.join('|',$blockterms).')%i';
	return ( preg_match( $blockterms_regexp, $user_agent )
					 || array_search( $user_agent, $blocklist ) );
}